-- Drop and recreate the cleanup function to also reset client journey stage
DROP FUNCTION IF EXISTS public.admin_cleanup_client_trainer_interactions(UUID, UUID);

CREATE FUNCTION public.admin_cleanup_client_trainer_interactions(
  p_client_id UUID,
  p_trainer_id UUID
)
RETURNS JSON
...
BEGIN
  ...
  -- 1. Delete messages between client and trainer
  WITH deleted_messages AS (
    DELETE FROM messages 
    WHERE (sender_id = p_client_id AND recipient_id = p_trainer_id) 
       OR (sender_id = p_trainer_id AND recipient_id = p_client_id)
    RETURNING id
  )
  ...
  WITH deleted_conversations AS (
    ...
    WHERE (participant1_id = p_client_id AND participant2_id = p_trainer_id) 
       OR (participant1_id = p_trainer_id AND participant2_id = p_client_id)
    RETURNING id
  )
  ...
  -- 6. Delete discovery call feedback notifications
  WITH deleted_feedback_notifications AS (
    DELETE FROM discovery_call_feedback_notifications dcfn
    USING discovery_call_feedback dcf
    WHERE dcfn.discovery_call_feedback_id = dcf.id 
      AND dcf.client_id = p_client_id 
      AND dcf.trainer_id = p_trainer_id
    RETURNING dcfn.id
  )
  ...
  WITH deleted_call_notes AS (
    DELETE FROM discovery_call_notes dcn
    USING discovery_calls dc
    WHERE dcn.discovery_call_id = dc.id 
      AND dc.client_id = p_client_id 
      AND dc.trainer_id = p_trainer_id
  ...
  -- 11. Delete commitment acknowledgments
  ...
  WITH deleted_alerts AS (
    ...
    WHERE ua.user_id = p_client_id 
      AND ua.alert_data::text LIKE '%' || p_trainer_id::text || '%'
    RETURNING ua.id
  )
  ...
  -- 21. RESET CLIENT JOURNEY STAGE - This is the new addition
  -- Reset client_journey_stage to NULL so it gets recalculated from actual data
  ...
  -- Return counts (including the new journey stage reset count)
