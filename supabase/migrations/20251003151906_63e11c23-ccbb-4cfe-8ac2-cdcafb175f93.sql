-- Update client_can_view_trainer_images function to include getting_to_know_your_coach stage
...
BEGIN
  ...
  -- NOW INCLUDING 'getting_to_know_your_coach' stage so saved trainers show gallery images
  RETURN EXISTS (
    SELECT 1 FROM client_trainer_engagement 
    WHERE client_id = current_user_id 
    AND trainer_id = p_trainer_id
    AND stage IN ('getting_to_know_your_coach', 'liked', 'shortlisted', 'discovery_call_booked', 'discovery_in_progress', 'discovery_completed', 'waitlist', 'active_client')
  ) OR EXISTS (
