-- Fix the admin_update_verification_check function to use correct enum values for verification_audit_action
-- Valid values are: delete, expire, reject, toggle_preference, upload, verify
...
BEGIN
  ...
  -- Log the action in audit log with correct enum values
  ...
  VALUES (
    ...
    auth.uid(),
    CASE 
      WHEN p_status = 'verified' THEN 'verify'::verification_audit_action
      WHEN p_status = 'rejected' THEN 'reject'::verification_audit_action
      ELSE 'verify'::verification_audit_action -- Default to verify for other statuses
    END,
    check_record.status,
    p_status,
