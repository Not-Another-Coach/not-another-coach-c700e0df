...
-- Create function to get available plans for a trainer
...
BEGIN
  ...
  SELECT tm.plan_type, tm.renewal_date
  INTO current_plan_type, current_renewal_date
  ...
  SELECT 
    ...
    CASE 
      ...
      WHEN mpd.monthly_price_cents > COALESCE((SELECT monthly_price_cents FROM membership_plan_definitions WHERE plan_type = current_plan_type), 0) THEN 'upgrade'
      ELSE 'downgrade'
    END as switch_type,
    NULL::TEXT as blocked_reason,
    CASE 
      WHEN current_renewal_date IS NOT NULL AND mpd.monthly_price_cents > COALESCE((SELECT monthly_price_cents FROM membership_plan_definitions WHERE plan_type = current_plan_type), 0)
      THEN ROUND((mpd.monthly_price_cents - COALESCE((SELECT monthly_price_cents FROM membership_plan_definitions WHERE plan_type = current_plan_type), 0)) * days_remaining / days_in_cycle)
      ELSE 0
