-- Update v_trainers view to include preferred_client_experience_levels
CREATE OR REPLACE VIEW public.v_trainers AS
SELECT 
    p.id,
    p.user_type,
    p.first_name,
    ...
    tp.preferred_client_experience_levels
FROM profiles p
LEFT JOIN trainer_profiles tp ON p.id = tp.id
WHERE p.user_type = 'trainer'::user_type;