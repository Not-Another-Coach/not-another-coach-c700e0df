-- Drop and recreate the function with the parameter order that matches the frontend call
...
GRANT EXECUTE ON FUNCTION public.create_coach_selection_request(uuid, text, text, numeric, text, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.create_coach_selection_request(uuid, text, text, numeric, text, text) TO anon;

-- Also let's check if there are any RLS policies blocking the insert
-- First, let's see the current RLS policies on coach_selection_requests
SELECT schemaname, tablename, policyname, permissive, roles, cmd, qual, with_check 
FROM pg_policies 
WHERE tablename = 'coach_selection_requests';