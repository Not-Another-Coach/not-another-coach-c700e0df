-- Update the update_client_profile_combined function to support new fields
CREATE OR REPLACE FUNCTION public.update_client_profile_combined(p_profile_data jsonb DEFAULT '{}'::jsonb, p_client_data jsonb DEFAULT '{}'::jsonb)
 RETURNS void
 LANGUAGE plpgsql
...
BEGIN
  ...
  IF p_client_data IS NOT NULL AND p_client_data != '{}'::jsonb THEN
    ...
    UPDATE client_profiles SET
      ...
      -- New fields for trainer preferences
      trainer_gender_preference = CASE WHEN p_client_data ? 'trainer_gender_preference' THEN (p_client_data->>'trainer_gender_preference')::trainer_gender_preference ELSE trainer_gender_preference END,
      discovery_call_preference = CASE WHEN p_client_data ? 'discovery_call_preference' THEN (p_client_data->>'discovery_call_preference')::discovery_call_preference ELSE discovery_call_preference END,
      updated_at = now()
    WHERE id = v_user_id;
  END IF;
END;
