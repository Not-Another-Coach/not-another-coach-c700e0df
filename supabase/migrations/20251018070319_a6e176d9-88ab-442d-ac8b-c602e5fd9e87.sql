-- Add cancel_at_period_end column to trainer_membership
ALTER TABLE trainer_membership 
ADD COLUMN IF NOT EXISTS cancel_at_period_end BOOLEAN DEFAULT false,
ADD COLUMN IF NOT EXISTS cancellation_grace_end DATE;

-- Fix cancel_trainer_plan function to use plan_definition_id instead of plan_type
DROP FUNCTION IF EXISTS cancel_trainer_plan(text);
...
DECLARE
  ...
  v_history_id UUID;
BEGIN
  -- Get current membership using plan_definition_id
  ...
  SET 
    ...
    cancellation_grace_end = renewal_date,
    updated_at = now()
