...
-- Update existing onboarding progress to use new completion tracking
...
SELECT 
  ...
  cop.client_id,
  cop.trainer_id, 
  cop.template_assignment_id,
  'task' as activity_type,
  jsonb_build_object(
    'notes', cop.notes,
    'file_url', cop.file_url,
    'legacy_migration', true
  ) as completion_data,
  cop.completed_at,
  CASE 
    WHEN cop.status = 'completed' THEN 'completed'
    WHEN cop.status = 'skipped' THEN 'skipped'
    ELSE 'pending'
  END as status,
  cop.due_at,
  cop.sla_due_at
FROM client_onboarding_progress cop
WHERE NOT EXISTS (
  SELECT 1 FROM activity_completions ac 
  WHERE ac.activity_id = cop.activity_id 
    AND ac.client_id = cop.client_id
    AND ac.template_assignment_id = cop.template_assignment_id
);