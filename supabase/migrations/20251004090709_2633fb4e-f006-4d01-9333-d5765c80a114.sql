-- Fix admin_cleanup_client_trainer_interactions to handle payment_packages foreign key
...
DECLARE
  ...
  payment_packages_count INTEGER := 0;
...
BEGIN
  ...
  -- 9. Delete payment packages (BEFORE deleting coach selection requests due to FK)
  WITH deleted_payment_packages AS (
    DELETE FROM payment_packages pp
    USING coach_selection_requests csr
    WHERE pp.coach_selection_request_id = csr.id
      AND csr.client_id = p_client_id 
      AND csr.trainer_id = p_trainer_id
    RETURNING pp.id
  )
  SELECT COUNT(*) INTO payment_packages_count FROM deleted_payment_packages;

  -- 10. Delete coach selection requests (AFTER payment packages)
  ...
  -- 12. Delete commitment acknowledgments
  BEGIN
  ...
  -- 13. Delete getting started progress
  ...
  -- 15. Delete template assignments
  ...
  -- 16. Delete ongoing support agreements
  ...
  -- 17. Delete conditional evaluations
  ...
  -- 19. Delete Instagram revelations
  ...
  -- 20. Skip alerts (no user_alerts table)
  alerts_count := 0;

  -- 21. Delete engagement records
  ...
  VALUES (
    ...
    json_build_object(
      ...
      'deleted_counts', json_build_object(
        ...
        'payment_packages', payment_packages_count,
