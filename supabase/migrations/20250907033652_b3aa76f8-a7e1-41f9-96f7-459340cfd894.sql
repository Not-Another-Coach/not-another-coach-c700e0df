-- Security Fixes Migration (Schema-Aware)
-- This migration addresses critical security vulnerabilities with correct table structures

-- 1. Fix trainer_uploaded_images table (verified to exist with trainer_id)
...
-- 2. Fix customer_payments table (use package_id to determine access)
...
-- 3. Fix discovery_call_feedback_questions table
...
DROP POLICY IF EXISTS "Safe authenticated users can view feedback questions" ON public.discovery_call_feedback_questions;
DROP POLICY IF EXISTS "Safe admins can manage feedback questions" ON public.discovery_call_feedback_questions;

-- Users need to be authenticated to view feedback questions
CREATE POLICY "Auth users can view active feedback questions"
ON public.discovery_call_feedback_questions FOR SELECT
TO authenticated
USING (is_archived = false);

-- Only admins can manage feedback questions
CREATE POLICY "Auth admins can manage feedback questions"
ON public.discovery_call_feedback_questions FOR ALL
TO authenticated
USING (has_role(auth.uid(), 'admin'::app_role))
WITH CHECK (has_role(auth.uid(), 'admin'::app_role));

-- 4. Fix payment_packages table
...
DROP POLICY IF EXISTS "Safe authenticated users can view active packages" ON public.payment_packages;
DROP POLICY IF EXISTS "Safe trainers can manage their packages" ON public.payment_packages;

-- Authenticated users can view active packages
CREATE POLICY "Auth users can view active packages"
ON public.payment_packages FOR SELECT
TO authenticated
USING (is_active = true);

-- Only trainers can manage their own packages
CREATE POLICY "Auth trainers can manage their packages"
...
DROP POLICY IF EXISTS "Safe clients can manage their selection requests" ON public.coach_selection_requests;
DROP POLICY IF EXISTS "Safe trainers can view and respond to their requests" ON public.coach_selection_requests;
DROP POLICY IF EXISTS "Safe trainers can update their requests" ON public.coach_selection_requests;

CREATE POLICY "Auth clients can manage their selection requests"
ON public.coach_selection_requests FOR ALL
TO authenticated
USING (client_id = auth.uid())
WITH CHECK (client_id = auth.uid());

CREATE POLICY "Auth trainers can view and respond to their requests"
ON public.coach_selection_requests FOR SELECT
TO authenticated
USING (trainer_id = auth.uid());

CREATE POLICY "Auth trainers can update their requests"
ON public.coach_selection_requests FOR UPDATE
TO authenticated
USING (trainer_id = auth.uid());

-- 6. Fix profiles table access control
DROP POLICY IF EXISTS "Public profiles are viewable by everyone" ON public.profiles;
DROP POLICY IF EXISTS "Authenticated users can view published profiles" ON public.profiles;
DROP POLICY IF EXISTS "Safe authenticated users can view published profiles" ON public.profiles;

CREATE POLICY "Auth users can view published profiles"
...
DROP POLICY IF EXISTS "Safe authenticated users can view active training types" ON public.training_types;

CREATE POLICY "Auth users can view active training types"
...
DROP POLICY IF EXISTS "Safe authenticated users can view active qualifications" ON public.popular_qualifications;

CREATE POLICY "Auth users can view active qualifications"
ON public.popular_qualifications FOR SELECT
TO authenticated
USING (is_active = true);

-- 9. Fix database functions - Add SET search_path to commonly used functions
